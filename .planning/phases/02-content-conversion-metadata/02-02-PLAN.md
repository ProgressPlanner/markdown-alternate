...
- render() method generates YAML frontmatter + converted body
- Featured image field omitted when post has no featured image
- Categories/tags formatted as YAML arrays when present
...
1. Visit `/any-post-slug.md` - should return YAML frontmatter + converted body
2. Check frontmatter includes: title, date, author
3. Post with featured image: frontmatter includes `featured_image:` field
4. Post without featured image: no `featured_image:` field in output
5. Post with categories: frontmatter includes `categories:` array
6. Post with shortcodes: shortcodes render (no `[shortcode]` in output)
7. Gutenberg post: blocks render (no `<!-- wp:` comments in output)
</verification>
...
- [ ] YAML frontmatter includes title, date, author (always)
- [ ] YAML frontmatter includes featured_image only when set
- [ ] YAML frontmatter includes categories/tags as arrays when present
